...
function sanitizePathForDisplay(filePath) {
...
function sourceMessage(code, filePath) {
  const displayPath = sanitizePathForDisplay(filePath);
  if (code === "SOURCE_NOT_FOUND") {
    return `源文件不存在：${displayPath}`;
  }
  if (code === "SOURCE_PARENT_DENIED") {
    return `应用无法遍历源文件所在目录：${displayPath}`;
  }
  if (code === "SOURCE_FILE_DENIED") {
    return `应用无法读取源文件：${displayPath}（请检查现有文件 ACL，目录授权不等于文件 ACL 已更新）`;
  }
  if (code === "SOURCE_REALPATH_FAILED") {
    return `无法解析源文件真实路径：${displayPath}`;
  }
  return `源路径不可用：${displayPath}`;
}
...
function inspectSourceFile(filePath, options = {}) {
  ...
  const pathParts = pathComponents(resolvedPath, pathModule);
