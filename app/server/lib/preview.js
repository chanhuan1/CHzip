...
function parseTechnicalList(text, options = {}) {
  ...
  if (byteLength > STREAM_PARSE_THRESHOLD_BYTES) {
    return parseTechnicalListStreaming(text, maxEntries);
  }

  return parseTechnicalListBatch(text, maxEntries);
}

function parseTechnicalListBatch(text, maxEntries) {
  ...
  return buildEntriesAndSummary(records);
}

function parseTechnicalListStreaming(text, maxEntries) {
  const summary = {
    fileCount: 0,
    directoryCount: 0,
    totalSize: 0,
  ...
  if (recordLines.length && foundSeparator) {
    const record = parseRecord(recordLines);
    if (record.Path) {
      const entry = buildEntry(record);
      entries.push(entry);
...
function updateSummary(summary, entry) {
  ...
  summary.encrypted ||= entry.encrypted;
}

function buildEntriesAndSummary(records) {
