...
function toJobView(job) {
  return {
    ...
    // 对外扩展字段（F2/F6/F7）。旧 job JSON 缺字段时用 || 默认值兜底。
    // conflictPolicy/outputStem 是内部字段，刻意不透传给前端。
    ...
    deleteSource: Boolean(job.deleteSource),
    deletedSourceCount: job.deletedSourceCount || 0,
    deleteSourceNote: job.deleteSourceNote || "",
    error: job.error
...
function createServices(options = {}) {
  ...
  // 统一的「为一个已创建 job 启动 worker」入口（F7 排队唤起复用）。
  ...
  function extract(input) {
    ...
    try {
      ...
      job = store.create({
        ...
        deleteSource: Boolean(input.deleteSource && !jobSelection),
