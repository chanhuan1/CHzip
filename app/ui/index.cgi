...
#    例如：/cgi/ThirdParty/CHzip/index.cgi/index.html?foo=bar
...
case "$URI_NO_QUERY" in
    *index.cgi*)
        ...
        # /cgi/ThirdParty/CHzip/index.cgi/index.html -> /index.html
...
case "$REL_PATH" in
    *..*)
        send_text "400 Bad Request" "Bad Request"
        exit 0
        ;;
esac
...
if [ ! -f "$TARGET_FILE" ]; then
    send_text "404 Not Found" "404 Not Found: ${REL_PATH}"
    exit 0
fi
...
# HTML/JS 不缓存（JS 通过资源 URL ?v= 版本号更新，避免旧包 JS 被浏览器长期缓存）
case "$ext" in
    html|htm|js)
